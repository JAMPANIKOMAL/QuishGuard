...
def build_app():
    print("\n[1/3] Building Core Application (Using Verified Command)...")
    # This uses the EXACT command you confirmed works
    subprocess.check_call([
        ...
        "--collect-all=pyzbar", # <--- The Critical Fix
...
def uninstall():
    ...
    # Dark Mode Message Box logic is tricky, standard native box is safer/cleaner for uninstalls
    ...
    if reply == QMessageBox.StandardButton.Yes:
        try:
            ...
            # 2. Delete Folder Content
            install_dir = os.path.dirname(sys.executable)
            for item in os.listdir(install_dir):
                if item.lower() != "uninstall.exe": # Can't delete self
                    try:
...
class InstallerWindow(QWidget):
    def __init__(self):
        ...
        # Dark Mode Style for Installer
        self.setStyleSheet(\"\"\"
            QWidget {{ background-color: #121212; color: white; font-family: Segoe UI; }}
            QLineEdit {{ padding: 8px; background: #222; border: 1px solid #444; color: #CCC; border-radius: 4px; }}
            QPushButton {{ background: #222; border: 1px solid #444; padding: 8px; font-weight: bold; border-radius: 4px; }}
            QPushButton:hover {{ background: #333; border-color: #00FF00; color: white; }}
            QProgressBar {{ border: 1px solid #444; text-align: center; border-radius: 4px; }}
